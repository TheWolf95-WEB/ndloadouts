...
// === Глобальный возврат на предыдущий экран ===
window.goBack = function() {
  ...
  if (!next) {
    console.warn('⚠️ Не найден предыдущий экран:', prev);
    return;
  }

  // Анимация ухода текущего экрана вправо
  if (current) {
    current.style.transition = 'transform 0.2s ease-out, opacity 0.2s ease-out';
  ...
  next.style.transition = 'transform 0.2s ease-out, opacity 0.2s ease-out';
  next.style.transform = 'translateX(0)';
  next.style.opacity = '1';

  window.scrollTo({ top: 0 });
  console.log('⬅️ Возврат на экран:', prev);
};
